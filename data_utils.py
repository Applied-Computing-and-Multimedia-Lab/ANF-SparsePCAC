...
def isin(data, ground_truth, attribute=False):
...
def istopk(data, nums, rho=1.0):
    ...
    mask1 = torch.zeros(len(data), dtype=torch.bool)
    mask2 = torch.zeros(len(data), dtype=torch.bool)
    mask3 = torch.zeros(len(data), dtype=torch.bool)
    ...
    for row_indices, N in zip(row_indices_per_batch, nums):
        ...
        elif data.F.size(1) == 3:
            _, indices1 = torch.topk((data.F)[:, 0:1][row_indices].squeeze().detach().cpu(), k)
            mask1[row_indices[indices1]] = True
            _, indices2 = torch.topk((data.F)[:, 1:2][row_indices].squeeze().detach().cpu(), k)
            mask2[row_indices[indices2]] = True
            _, indices3 = torch.topk((data.F)[:, 2:3][row_indices].squeeze().detach().cpu(), k)
            mask3[row_indices[indices3]] = True
            mask = torch.stack((mask1, mask2, mask3), 1)
