...
def estimate_bpp(likelihood, num_cube, num_pixels=None, input=None, likelihood_bound: float = 1e-9):
    ...
    return lll / (-np.log(2.) * num_cube)

def estimate_bpp_eval(likelihood, num_pixels=None, input=None, likelihood_bound: float = 1e-9):
    """estimate bits-per-pixel
...
class EntropyBottleneck(EntropyModel):
    ...
    def _logits_cumulative(self, input, detach=False):
        ...
        """
        xx = self.factorizer(input, detach=detach)
        return xx
